...
class Cat13 {
    ...
    public function crear() {
        try {
            $query = "INSERT INTO cat13 (id_cat13, id_cat12, vc_codigo, vc_valores, d_registro, d_actualizacion, c_activo) VALUES (:id_cat13, :id_cat12, :vc_codigo, :vc_valores, :d_registro, :d_actualizacion, :c_activo)";
    ...
    public function leerUno($id) {
        try {
            $query = "SELECT * FROM cat13 WHERE id = :id LIMIT 0,1";
            $stmt = $this->conn->prepare($query);
            $params = [':id' => $id];
            $stmt->execute($params);
            $row = $stmt->fetch(PDO::FETCH_ASSOC);
            if ($row) {
                $this->id_cat13 = $row['id_cat13'];
    ...
    public function actualizar() {
        try {
            $query = "UPDATE cat13 SET id_cat13 = : , id_cat12 = : , vc_codigo = : , vc_valores = : , d_registro = : , d_actualizacion = : , c_activo = : WHERE id = :id";
            $stmt = $this->conn->prepare($query);
    ...
    public function eliminar($id) {
        try {
            $query = "DELETE FROM cat13 WHERE id = :id";
            $stmt = $this->conn->prepare($query);
            $params = [':id' => $id];
            if ($stmt->execute($params)) {
                return true;
