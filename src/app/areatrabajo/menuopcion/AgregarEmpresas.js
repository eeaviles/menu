...
//-----[REDUX TOOLKIT]-----
import { useEnvrempMutation } from "../../redux/servicio/GenericApi";
...
const AgregarEmpresas = () => {
  ...
  const { IDPEmpresaSelec } = useSelector(principalSelector);

  console.log(IDPEmpresaSelec);
  ...
  return (
    <>
      <div className="container mx-auto contenedor contenedorcentral">
        {IDPEmpresaSelec !== null ? (
          <div className="Titulo">Agregar Empresa / Persona Jurídica</div>
        ) : (
          <div className="Titulo">Agregar Datos Empresa Principal</div>
        )}
          ...
          <Row className="ATBJformfila">
            <Col xs={12} sm={12} md={12} lg={12} xl={12}>
              <Form.Group controlId="terciaria">
                <label htmlFor="terciaria" className="form-label ATBJformLabel">Terciaria:</label>
                <input
                  required
                  type="text"
                  placeholder="Buscar"
