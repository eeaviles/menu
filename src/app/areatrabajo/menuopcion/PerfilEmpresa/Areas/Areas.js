...
//-----[REDUX TOOLKIT]-----
import { useObtareasQuery } from "../../../../redux/servicio/GenericApi";
...
import AreasAcciones from "./AreasAcciones";
...
//---[ TABLA SUCURSALES HELPERS ]-------------------------
  ...
  const COL_AREAS = [
    ...
    columnHelper.accessor("CODAREA", {
      header: () => alineacion("text", "left", "CÓDIGO"),
      cell: (info) => alineacion("text", "left", info.renderValue()),
    }),
    columnHelper.accessor("ANBRE", {
      header: () => alineacion("text", "left", "ÁREA DE TRABAJO"),
      cell: (info) => alineacion("text", "left", info.renderValue()),
    }),
...
//---[COMPONENTE PRINCIPALES]----------------------------------------------------------
  const Areas = ({ IDPEmpresaSelec }) => { 

    //---[ QUERY: OBTENER AREAS DE TRABAJO ]----
    ...
    const { data: ListaAreas, isSuccess: isListaAreas } = useObtareasQuery(OBtcam,{ skip: !OBtcam }); 

    return (
      <>
        {isListaAreas !== false && (
          <div className="container mx-auto contenedor">
            <div className="Titulo">LISTA DE AREAS DE TRABAJO</div>
            {ListaAreas && (
              <TablaReporte
                IDEMPRE={IDPEmpresaSelec}
                Filas={ListaAreas}
                TIPOCOLUMNA={"COL_AREAS"}
                ETIQUETA="AREAS"
    ...
    return (
      <>
        ...
        <div className="ContBusquedaAgregar">
          <FiltroGlobal filter={globalFilter} setFilter={setGlobalFilter} tex='a'/>
