...
//-----[REDUX TOOLKIT]-----
import { useObtcategoriasQuery } from "../../../../redux/servicio/GenericApi";
...
import CategoriasAcciones from "./CategoriasAcciones";

//---[ TABLA CATEGORIAS HELPERS ]-------------------------
  ...
  const COL_CATEGORIAS= [
    ...
    columnHelper.accessor("TIPOCAT", {
      header: () => alineacion("text", "left", "TIPOCATEGORÍA"),
      cell: (info) =>  {
        const value = info.renderValue();
        const mappedValue = value === "P" ? "Programado" : value === "G" ? "Generico" : value;
        return alineacion("text", "left", mappedValue);
      },
    }),
    columnHelper.accessor("DESCCAT", {
...
//---[COMPONENTE PRINCIPALES]----------------------------------------------------------
  const Categorias = ({ IDPEmpresaSelec }) => {
    ...
    const { data: ListaCategorias, isSuccess: isListaCategorias } = useObtcategoriasQuery(OBtcam,{ skip: !OBtcam }); 
    return (
      <>
        {isListaCategorias !== false && (
          <div className="container mx-auto contenedor">
            <div className="Titulo">LISTA DE CATEGORÍAS</div>
            {ListaCategorias && (
              <TablaReporte
                IDEMPRE={IDPEmpresaSelec}
                Filas={ListaCategorias}
                TIPOCOLUMNA={COL_CATEGORIAS}
                ETIQUETA="CATEGORÍAS"
                COMPONENTE={CategoriasAcciones}
              />
            )}
          </div>
        )}
    ...
    return (
      <>
        ...
        <div className="ContBusquedaAgregar">
          <FiltroGlobal filter={globalFilter} setFilter={setGlobalFilter} tex='c'/>
