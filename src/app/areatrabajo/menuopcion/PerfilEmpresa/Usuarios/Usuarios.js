...
//-----[REDUX TOOLKIT]-----
import { useObtusuariosQuery } from "../../../../redux/servicio/GenericApi";
...
import UsuariosAcciones from "./UsuariosAcciones";
...
//---[ TABLA SUCURSALES HELPERS ]-------------------------
  ...
  const COL_USUARIOS = [
    ...
    columnHelper.accessor("NPER", {
        header: () => alineacion("text", "left", "NOMBRE"),
        cell: (info) => alineacion("text", "left", info.renderValue()),
      }),
    columnHelper.accessor("ROL", {
      header: () => alineacion("text", "left", "ROL"),
      cell: (info) => alineacion("text", "left", info.renderValue()),
    }),
    columnHelper.accessor("NUSER", {
      header: () => alineacion("text", "center", "login"),
      cell: (info) => alineacion("text", "left", info.renderValue()),
    }),
    ...
    columnHelper.accessor("USERACT", {
      header: () => alineacion("text", "center", "ACTIVO"),
      cell: (info) => alineacion("text", "left", info.renderValue()),
    }),
...
//---[COMPONENTE PRINCIPALES]----------------------------------------------------------
  const Usuarios = ({ IDPEmpresaSelec }) => {
    ...
    const { data: ListaUsuarios, isSuccess: isListaUsuarios } = useObtusuariosQuery(OBtcam,{ skip: !OBtcam }); 

    return (
      <>
        {isListaUsuarios !== false && (
          <div className="container mx-auto contenedor">
            <div className="Titulo">LISTA DE USUARIOS</div>
            {ListaUsuarios && (
              <TablaReporte
                IDEMPRE={IDPEmpresaSelec}
                Filas={ListaUsuarios}
                TIPOCOLUMNA={COL_USUARIOS}
                ETIQUETA="USUARIOS"
                COMPONENTE={UsuariosAcciones}
              />
            )}
          </div>
        )}
    ...
    return (
      <>
        ...
        <div className="ContBusquedaAgregar">
          <FiltroGlobal filter={globalFilter} setFilter={setGlobalFilter} tex='u'/>
