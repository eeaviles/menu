...
//-----[REDUX TOOLKIT]-----
import { useObtptbjoQuery } from "../../../../redux/servicio/GenericApi";
...
import AreasAcciones from "./PuestosTrabajosAcciones";
...
//---[COMPONENTE PRINCIPALES]----------------------------------------------------------
  const PuestosTrabajos = ({ IDPEmpresaSelec }) => { 

    //---[ QUERY: OBTENER PUESTOS DE TRABAJO ]----
    ...
    const { data: PuestosTrabajos, isSuccess: isPuestosTrabajos } = useObtptbjoQuery(OBtcam,{ skip: !OBtcam }); 

    return (
      <>
        {isPuestosTrabajos !== false && (
          <div className="container mx-auto contenedor">
            <div className="Titulo">LISTA DE PUESTOS DE TRABAJO</div>
            {PuestosTrabajos && (
              <TablaReporte
                IDEMPRE={IDPEmpresaSelec}
                Filas={PuestosTrabajos}
                TIPOCOLUMNA={"COL_PUESTOSTRABAJO"}
                ETIQUETA="PUESTOS DE TRABAJO"
                COMPONENTE={AreasAcciones}
              />
            )}
          </div>
        )}
...
//---[COMPONENTE TABLA]----------------------------------------------------------
  const TablaReporte = ({IDEMPRE, Filas, TIPOCOLUMNA, ETIQUETA, COMPONENTE}) => {
    ...
    const columns = TIPOCOLUMNA === "COL_PUESTOSTRABAJO" ? COL_PUESTOSTRABAJO : null; //---[ CARGAR COLUMNAS ]
    ...
    return (
      <>
        ...
        <div className="ContBusquedaAgregar">
          <FiltroGlobal filter={globalFilter} setFilter={setGlobalFilter} tex='pt'/>
